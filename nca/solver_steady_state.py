...
class NCA_Steady_State_Solver:
            ...
            for x in range(self.nr_orbitals):
                if self.is_orb_in_state(x, k):
                    if self.H_loc[k - 2**x] < np.inf:
                        self.S_grea_reta[:, k] += 1j * self.delta_grea_dict[x] \
                                            * self.R_grea_reta[:, k - 2**x]
                else:
                    if self.H_loc[k + 2**x] < np.inf:
                        self.S_grea_reta[:, k] += -1j * self.delta_less_dict[x][::-1] \
                                            * self.R_grea_reta[:, k + 2**x]
    ...
    def self_energy_less(self):
        ...
        for k in range(self.D):
            for x in range(self.nr_orbitals):
                if self.is_orb_in_state(x, k):
                    if self.H_loc[k - 2**x] < np.inf:
                        self.S_less[:, k] += -1j * self.delta_less_dict[x] \
                                            * self.R_less[:, k - 2**x]
                else:
                    if self.H_loc[k + 2**x] < np.inf:
                        self.S_less[:, k] += 1j * self.delta_grea_dict[x][::-1] \
                                            * self.R_less[:, k + 2**x]
    ...
    def normalize_less(self):
        Z = 1j * np.mean(self.R_less[len(self.time_mesh) // 2], where=(self.H_loc < np.inf))
        self.R_less_w /= Z
        self.R_less /= Z
    ...
    def lesser_loop(self, tol=1e-8, max_iter=100, plot=False, verbose=False):
        ...
        while (err > tol and n < max_iter):
            ...
            err = np.trapz(np.mean(np.abs(self.R_less_w - R_less_w_prev), axis=1), dx=self.freq_mesh.delta)
            if verbose:
                print(self.N2, err)
    ...
    def get_G_grea(self, orb):
        ...
        for state in range(self.D):
            if not self.is_orb_in_state(orb, state):
                G_grea += self.R_less[::-1, state] * \
                        self.R_grea[:, state + 2 ** orb]
    ...
    def get_G_less(self, orb):
        ...
        for state in range(self.D):
            if self.is_orb_in_state(orb, state):
                G_less += self.R_less[:, state] * \
                        self.R_grea[::-1, state - 2 ** orb]
